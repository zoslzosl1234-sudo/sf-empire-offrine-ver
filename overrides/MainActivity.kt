...
이제 원본 규칙에 따라:
1) 플레이어 모태양계를 생성하고,
2) 나머지 8종족의 독립 AI 문명을 비공개로 생성하고,
3) 초기 경제/함대/연구 상태를 생성하고,
4) 프롤로그를 짧게 출력한 뒤,
5) 2200년 1턴을 시작한다.
...
")
            append("사용 포인트: **$used/12** · 남은 포인트: **$remaining**

")

            traitNames.forEachIndexed { index, name ->
                append("${index + 1}. **$name** Lv.${traitLevels[index]} — ${traitDescriptions[index]}
")
            }
...
")
            append("- `1-` → 1번 특성 -1
")
            append("- `1:3` → 1번 특성을 Lv.3으로 설정
")
            append("- `1:3,2:2,4:2,6:3,8:2` → 여러 특성 한 번에 설정
")
            append("- `초기화` → 전부 Lv.0
")
            append("- 12포인트를 모두 사용한 뒤 `확정` → 다음 단계

")
            append("각 특성은 Lv.0~Lv.5, 전체 합계는 정확히 12입니다.")
        }
    }
...
- 게임은 2200년에 시작한다.
- 시작은 반드시 한 단계씩 진행한다.
- 첫 단계는 9종족 선택이다: 테란, 드라칸, 실리카, 메카리온, 벨카인, 네레이드, 아르보리안, 세라핌, 사이오니스.
- 종족 선택 후 국가명/정치체계, 초기 행성, 성장특성, 최종 확인 순으로 진행한다.\n- 시작 정치체계는 민주공화정, 대통령공화정, 입헌군주정, 전제군주정, 권위주의 독재정, 군사독재정, 공산주의정, 신정정치, 기업국가 9종이다. 집단의식정은 시작 선택 불가다.
