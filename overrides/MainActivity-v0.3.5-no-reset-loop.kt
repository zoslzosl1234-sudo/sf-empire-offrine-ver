package com.example.llama

import android.net.Uri
import android.os.Bundle
import android.util.Log
import android.widget.Button
import android.widget.EditText
import android.widget.TextView
import android.widget.Toast
import androidx.activity.addCallback
import androidx.activity.enableEdgeToEdge
import androidx.activity.result.contract.ActivityResultContracts
import androidx.appcompat.app.AppCompatActivity
import androidx.lifecycle.lifecycleScope
import androidx.recyclerview.widget.LinearLayoutManager
import androidx.recyclerview.widget.RecyclerView
import com.arm.aichat.AiChat
import com.arm.aichat.InferenceEngine
import com.arm.aichat.gguf.GgufMetadata
import com.arm.aichat.gguf.GgufMetadataReader
import com.google.android.material.floatingactionbutton.FloatingActionButton
import kotlinx.coroutines.Dispatchers
import kotlinx.coroutines.Job
import kotlinx.coroutines.flow.first
import kotlinx.coroutines.flow.onCompletion
import kotlinx.coroutines.launch
import kotlinx.coroutines.withContext
import org.json.JSONArray
import org.json.JSONObject
import java.io.File
import java.io.FileOutputStream
import java.io.InputStream
import java.util.UUID

class MainActivity : AppCompatActivity() {
    private lateinit var statusTv: TextView
    private lateinit var messagesRv: RecyclerView
    private lateinit var userInputEt: EditText
    private lateinit var userActionFab: FloatingActionButton
    private lateinit var selectModelBtn: Button
    private lateinit var saveBtn: Button
    private lateinit var loadBtn: Button
    private lateinit var newGameBtn: Button

    private lateinit var engine: InferenceEngine
    private var generationJob: Job? = null
    private var engineInitialized = false
    private var isModelReady = false
    private var currentModelPath: String? = null

    private val messages = mutableListOf<Message>()
    private val rawAssistant = StringBuilder()
    private var lastUiRenderMs = 0L
    private val messageAdapter = MessageAdapter(messages)

    private lateinit var coreRules: String
    private lateinit var fullRules: String
    private var needsCoreInjection = true
    private var needsResumeContext = false

    private enum class SetupStage {
        NONE, RACE, NATION_NAME, GOVERNMENT, PLANET, TRAITS, CONFIRM, PLAYING
    }

    private var setupStage = SetupStage.NONE
    private var selectedRace = ""
    private var nationName = ""
    private var selectedGovernment = ""
    private var selectedPlanet = ""
    private var selectedTraits = ""
    private val traitLevels = IntArray(8)
    private val traitNames = listOf(
        "적응", "번성", "강인", "지능",
        "산업", "사회/행정", "감각/정보", "에너지/회복"
    )
    private val traitDescriptions = listOf(
        "거주성·부적합 환경 유지비",
        "인구성장·이민·정착",
        "지상군 생존·피해저항",
        "연구·희귀 연구 발견",
        "IP·건설·생산",
        "안정·행정·LC 효율",
        "탐지·정찰·이상현상 발견",
        "에너지 생산·운용 또는 회복·재난저항"
    )

    private val prefs by lazy { getSharedPreferences(PREFS, MODE_PRIVATE) }

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        enableEdgeToEdge()
        setContentView(R.layout.activity_main)
        onBackPressedDispatcher.addCallback { Log.w(TAG, "Back press ignored during local inference") }

        statusTv = findViewById(R.id.status)
        messagesRv = findViewById(R.id.messages)
        userInputEt = findViewById(R.id.user_input)
        userActionFab = findViewById(R.id.fab)
        selectModelBtn = findViewById(R.id.select_model)
        saveBtn = findViewById(R.id.save_game)
        loadBtn = findViewById(R.id.load_game)
        newGameBtn = findViewById(R.id.new_game)

        messagesRv.layoutManager = LinearLayoutManager(this).apply { stackFromEnd = true }
        messagesRv.adapter = messageAdapter

        coreRules = readAssetText("sf_empire_core_rules.txt")
        fullRules = readAssetText("sf_empire_rules.txt")
        restoreAutosave()
        restoreSetupState()
        refreshButtons()

        selectModelBtn.setOnClickListener { getContent.launch(arrayOf("*/*")) }
        saveBtn.setOnClickListener { saveSnapshot() }
        loadBtn.setOnClickListener { loadSnapshot() }
        newGameBtn.setOnClickListener { startNewGame() }
        userActionFab.setOnClickListener { handleUserInput() }

        lifecycleScope.launch(Dispatchers.Default) {
            engine = AiChat.getInferenceEngine(applicationContext)
            engineInitialized = true

            val stableState = engine.state.first {
                it is InferenceEngine.State.Initialized ||
                it is InferenceEngine.State.ModelReady ||
                it is InferenceEngine.State.Error
            }

            val savedPath = prefs.getString(KEY_MODEL_PATH, null)

            when {
                stableState is InferenceEngine.State.ModelReady -> {
                    currentModelPath = savedPath
                    withContext(Dispatchers.Main) {
                        isModelReady = true
                        needsCoreInjection = true
                        if (messages.isNotEmpty()) needsResumeContext = true
                        setStatus("로컬 모델 준비 완료 · 완전 오프라인")
                        setIdleUi()
                    }
                }

                savedPath != null && File(savedPath).exists() -> {
                    currentModelPath = savedPath
                    loadModel(File(savedPath), auto = true)
                }

                else -> {
                    if (stableState is InferenceEngine.State.Error) {
                        runCatching { engine.cleanUp() }
                    }
                    withContext(Dispatchers.Main) {
                        setStatus("GGUF 모델을 선택하세요. 모든 추론은 기기 안에서 실행됩니다.")
                        refreshButtons()
                    }
                }
            }
        }
    }

    private val getContent = registerForActivityResult(ActivityResultContracts.OpenDocument()) { uri ->
        uri?.let { handleSelectedModel(it) }
    }

    private fun handleSelectedModel(uri: Uri) {
        if (!engineInitialized) {
            toast("AI 엔진 초기화 중입니다. 잠시 후 다시 시도하세요.")
            return
        }
        setBusyUi("GGUF 정보를 읽는 중…")
        lifecycleScope.launch(Dispatchers.IO) {
            try {
                val metadata = contentResolver.openInputStream(uri)?.use {
                    GgufMetadataReader.create().readStructuredMetadata(it)
                } ?: error("GGUF 파일을 읽지 못했습니다.")
                val modelName = metadata.filename() + FILE_EXTENSION_GGUF
                withContext(Dispatchers.Main) { setStatus("모델 준비 중: $modelName") }
                val modelFile = contentResolver.openInputStream(uri)?.use { input ->
                    ensureModelFile(modelName, input)
                } ?: error("모델 파일을 복사하지 못했습니다.")
                currentModelPath = modelFile.path
                prefs.edit().putString(KEY_MODEL_PATH, modelFile.path).apply()
                loadModel(modelFile, auto = false)
            } catch (t: Throwable) {
                Log.e(TAG, "Model import failed", t)
                withContext(Dispatchers.Main) {
                    setStatus("모델 불러오기 실패: ${t.message}")
                    setIdleUi()
                }
            }
        }
    }

    private suspend fun ensureModelFile(modelName: String, input: InputStream) = withContext(Dispatchers.IO) {
        File(ensureModelsDirectory(), modelName).also { file ->
            if (!file.exists() || file.length() == 0L) {
                withContext(Dispatchers.Main) { setStatus("모델을 앱 저장공간으로 복사 중… 처음 한 번은 오래 걸립니다.") }
                FileOutputStream(file).use { input.copyTo(it) }
            }
        }
    }

    private suspend fun loadModel(modelFile: File, auto: Boolean) = withContext(Dispatchers.IO) {
        try {
            withContext(Dispatchers.Main) { setBusyUi("로컬 AI 모델 로딩 중…") }

            val stableState = engine.state.first {
                it is InferenceEngine.State.Initialized ||
                it is InferenceEngine.State.ModelReady ||
                it is InferenceEngine.State.Error
            }

            if (stableState is InferenceEngine.State.ModelReady) {
                if (auto) {
                    currentModelPath = modelFile.path
                    withContext(Dispatchers.Main) {
                        isModelReady = true
                        needsCoreInjection = true
                        if (messages.isNotEmpty()) needsResumeContext = true
                        setStatus("로컬 모델 자동 복구 완료 · 완전 오프라인")
                        setIdleUi()
                    }
                    return@withContext
                } else {
                    engine.cleanUp()
                }
            } else if (stableState is InferenceEngine.State.Error) {
                engine.cleanUp()
            }

            engine.loadModel(modelFile.path)
            currentModelPath = modelFile.path

            withContext(Dispatchers.Main) {
                isModelReady = true
                needsCoreInjection = true
                if (messages.isNotEmpty()) needsResumeContext = true
                setStatus(if (auto) "로컬 모델 자동 로드 완료 · 완전 오프라인" else "로컬 모델 준비 완료 · 완전 오프라인")
                setIdleUi()
                if (messages.isEmpty()) {
                    addAssistant(
                        "모델이 준비되었습니다. ‘새 게임’을 누르면 SF 우주 제국 시뮬레이션을 시작합니다.",
                        persist = false
                    )
                }
            }
        } catch (t: Throwable) {
            Log.e(TAG, "Model load failed", t)
            withContext(Dispatchers.Main) {
                isModelReady = engine.state.value is InferenceEngine.State.ModelReady
                if (isModelReady) {
                    setStatus("로컬 모델 준비 완료 · 완전 오프라인")
                } else {
                    setStatus("모델 로딩 실패: ${t.message}")
                }
                setIdleUi()
            }
        }
    }

    private suspend fun resetLoadedModel(path: String) = withContext(Dispatchers.IO) {
        val stableState = engine.state.first {
            it is InferenceEngine.State.Initialized ||
            it is InferenceEngine.State.ModelReady ||
            it is InferenceEngine.State.Error
        }

        if (
            stableState is InferenceEngine.State.ModelReady ||
            stableState is InferenceEngine.State.Error
        ) {
            engine.cleanUp()
        }

        engine.loadModel(path)
    }

    private fun startNewGame() {
        if (!isModelReady) {
            toast("먼저 GGUF 모델을 선택하세요.")
            return
        }

        generationJob?.cancel()
        messages.clear()
        messageAdapter.notifyDataSetChanged()

        selectedRace = ""
        nationName = ""
        selectedGovernment = ""
        selectedPlanet = ""
        selectedTraits = ""
        traitLevels.fill(0)
        setupStage = SetupStage.RACE
        needsCoreInjection = true
        needsResumeContext = false
        userInputEt.text = null

        saveAutosave()
        saveSetupState()

        addAssistant(racePrompt())
        setStatus("국가 생성 · 종족 선택")
        setIdleUi()
    }

    private fun handleUserInput() {
        if (!isModelReady) {
            toast("먼저 GGUF 모델을 선택하세요.")
            return
        }

        val userMsg = userInputEt.text.toString().trim()
        if (userMsg.isEmpty()) return
        userInputEt.text = null

        if (setupStage != SetupStage.PLAYING && setupStage != SetupStage.NONE) {
            addMessage(Message(UUID.randomUUID().toString(), userMsg, true))
            handleSetupInput(userMsg)
            return
        }

        sendGameMessage(userMsg, displayUser = true)
    }

    private fun handleSetupInput(input: String) {
        when (setupStage) {
            SetupStage.RACE -> {
                val race = parseRace(input)
                if (race == null) {
                    addAssistant("종족을 인식하지 못했습니다. 1~9 번호 또는 종족명을 입력해주세요.\n\n${racePrompt()}")
                    return
                }

                selectedRace = race
                setupStage = SetupStage.NATION_NAME
                saveSetupState()

                addAssistant(
                    "**$selectedRace** 선택이 확정되었습니다.\n\n" +
                    "국가 이름을 입력해주세요.\n예: 아쿠아 연방, 심해 제국, 자유행성연합"
                )
                setStatus("국가 생성 · 국가명")
            }

            SetupStage.NATION_NAME -> {
                if (input.length < 2) {
                    addAssistant("국가 이름을 2글자 이상 입력해주세요.")
                    return
                }

                nationName = input.take(40)
                setupStage = SetupStage.GOVERNMENT
                saveSetupState()

                addAssistant(governmentPrompt())
                setStatus("국가 생성 · 정치체계")
            }

            SetupStage.GOVERNMENT -> {
                val government = parseGovernment(input)
                if (government == null) {
                    addAssistant("정치체계를 인식하지 못했습니다. 1~9 번호 또는 이름을 입력해주세요.\n\n${governmentPrompt()}")
                    return
                }

                selectedGovernment = government
                setupStage = SetupStage.PLANET
                saveSetupState()

                addAssistant(planetPrompt(selectedRace))
                setStatus("국가 생성 · 시작 행성")
            }

            SetupStage.PLANET -> {
                val planet = parsePlanet(input, selectedRace)
                if (planet == null) {
                    addAssistant("행성 조건을 인식하지 못했습니다. 행성 유형이나 환경을 2글자 이상 입력해주세요.\n\n${planetPrompt(selectedRace)}")
                    return
                }

                selectedPlanet = planet
                setupStage = SetupStage.TRAITS
                saveSetupState()

                addAssistant(traitsPrompt())
                setStatus("국가 생성 · 추가 특성")
            }

            SetupStage.TRAITS -> {
                when (handleTraitCommand(input)) {
                    TraitCommandResult.UPDATED -> {
                        selectedTraits = traitSummary()
                        saveSetupState()
                        addAssistant(traitsPrompt())
                        setStatus("국가 생성 · 성장특성 ${usedTraitPoints()}/12")
                    }

                    TraitCommandResult.CONFIRMED -> {
                        selectedTraits = traitSummary()
                        setupStage = SetupStage.CONFIRM
                        saveSetupState()
                        addAssistant(confirmPrompt())
                        setStatus("국가 생성 · 최종 확인")
                    }

                    TraitCommandResult.INVALID -> {
                        addAssistant(
                            "입력을 인식하지 못했거나 포인트 조건이 맞지 않습니다.\n\n" +
                            traitsPrompt()
                        )
                    }
                }
            }

            SetupStage.CONFIRM -> {
                val normalized = input.trim().lowercase()

                if (
                    normalized == "1" ||
                    normalized == "확정" ||
                    normalized == "확인" ||
                    normalized == "시작"
                ) {
                    setupStage = SetupStage.PLAYING
                    saveSetupState()
                    needsCoreInjection = true
                    needsResumeContext = false

                    val setup = confirmedSetupSummary()

                    sendGameMessage(
                        """
국가 설정이 최종 확정되었다.
아래 확정값은 절대로 변경하지 않는다.

$setup

이 명령은 캠페인 최초 1회 초기화 명령이다.
원본 규칙에 따라:
1) 플레이어 모태양계를 1회 생성하고,
2) 나머지 8종족의 독립 AI 문명을 비공개로 1회 초기화하고,
3) 초기 경제/함대/연구 상태를 1회 생성하고,
4) 프롤로그를 짧게 출력하고,
5) 2200년 1턴을 시작한다.

이 응답 이후에는 국가 생성, 세계 초기화, 프롤로그를 절대로 반복하지 않는다.
미접촉 AI 문명의 상세 정보는 플레이어에게 공개하지 않는다.
""".trimIndent(),
                        displayUser = false
                    )
                } else if (
                    normalized == "2" ||
                    normalized.contains("수정") ||
                    normalized.contains("다시")
                ) {
                    setupStage = SetupStage.RACE
                    selectedRace = ""
                    nationName = ""
                    selectedGovernment = ""
                    selectedPlanet = ""
                    selectedTraits = ""
                    traitLevels.fill(0)
                    saveSetupState()
                    addAssistant(racePrompt())
                    setStatus("국가 생성 · 종족 선택")
                } else {
                    addAssistant("`1` 또는 `확정`을 입력하면 시작합니다.\n`2` 또는 `수정`을 입력하면 종족 선택부터 다시 설정합니다.")
                }
            }

            else -> Unit
        }
    }

    private fun racePrompt(): String = """
### 종족 선택 · v6.5

1. **테란** — 인간형 탄소생명 / 범용·유연성
2. **드라칸** — 용형 파충류 / 신체·정면전
3. **실리카** — 광물 생명체 / 내구·극한환경
4. **메카리온** — 기계융합 생명체 / 기계·산업·에너지
5. **벨카인** — 포유류형 수인 / 감각·기동·정찰
6. **네레이드** — 수생 지성체 / 해양·심해·고압
7. **아르보리안** — 식물형 생명체 / 생태·재생·자급
8. **세라핌** — 부유형 기체생명체 / 대기·공중·입체환경
9. **사이오니스** — 정신공명형 유기생명체 / 공명·협동·지휘

번호 또는 종족명을 입력하세요.
""".trimIndent()

    private fun governmentPrompt(): String = """
**$nationName**의 시작 정치체계를 선택해주세요.

1. **민주공화정** — 시민대표·선거 / 안정·경제·외교 / 정책결정 지연
2. **대통령공화정** — 강한 행정부 / 균형·위기대응 / 권력갈등
3. **입헌군주정** — 군주+의회 / 장기안정·통합 / 개혁속도
4. **전제군주정** — 군주 중심 / 빠른 결정·동원 / 후계·폭정 위험
5. **권위주의 독재정** — 중앙집권 / 통제·산업동원 / 자유·불만
6. **군사독재정** — 군부 지배 / 군사·동원·방위 / 민간경제·외교
7. **공산주의정** — 집단 생산·계획경제 / 산업·건설·배분 / 계획 경직성
8. **신정정치** — 종교·신념 중심 / 결속·사기 / 과학·사회갈등 가능
9. **기업국가** — 기업 중심 / 산업·무역·개발 / 불평등·충성도

※ 집단의식정은 시작 선택지가 아니며 후반 조건부 해금 체계입니다.

번호 또는 정치체계 이름을 입력하세요.
""".trimIndent()

    private fun planetPrompt(race: String): String = """
### 초기 행성 선택

선택 종족: **$race**

v6.5 규칙에서는 종족이 실제로 거주 가능한 후보 행성을 제시하고,
후보마다 다음 항목을 비교한 뒤 선택합니다.

- 행성유형
- 크기
- 환경
- 초기인구
- 자원
- 거주성
- 장점 / 단점
- 특수효과

현재는 원하는 초기 행성 유형이나 환경을 직접 입력해주세요.
예: `해양형`, `고중력 화산형`, `대륙형`

입력한 조건은 확정값으로 저장되며, 게임 시작 시 GM이 v6.5 거주성 규칙에 맞는 세부 행성을 생성합니다.
""".trimIndent()

    private fun traitsPrompt(): String {
        val used = usedTraitPoints()
        val remaining = 12 - used

        val rows = traitNames.indices.joinToString("\n") { index ->
            "${index + 1}. **${traitNames[index]}** Lv.${traitLevels[index]} — ${traitDescriptions[index]}"
        }

        return """
### 성장특성 분배 · v6.5

사용 포인트: **$used/12** · 남은 포인트: **$remaining**

$rows

입력 방법:
- `1+` → 1번 특성 +1
- `1-` → 1번 특성 -1
- `1:3` → 1번 특성을 Lv.3으로 설정
- `1:3,2:2,4:2,6:3,8:2` → 여러 특성 한 번에 설정
- `초기화` → 전부 Lv.0
- 12포인트를 모두 사용한 뒤 `확정` → 다음 단계

각 특성은 Lv.0~Lv.5, 전체 합계는 정확히 12입니다.
""".trimIndent()
    }

    private fun confirmPrompt(): String = """
### 국가 설정 최종 확인

${confirmedSetupSummary()}

`1` 또는 `확정` — 이 설정으로 게임 시작
`2` 또는 `수정` — 종족 선택부터 다시 설정
""".trimIndent()

    private fun confirmedSetupSummary(): String = """
- 국가명: $nationName
- 종족: $selectedRace
- 정치체계: $selectedGovernment
- 시작 행성 유형: $selectedPlanet
- 추가 특성: $selectedTraits
""".trimIndent()

    private fun parseRace(input: String): String? {
        val races = listOf(
            "테란", "드라칸", "실리카", "메카리온", "벨카인",
            "네레이드", "아르보리안", "세라핌", "사이오니스"
        )
        val t = input.trim()
        val n = t.toIntOrNull()
        if (n != null && n in 1..races.size) return races[n - 1]
        return races.firstOrNull { t.contains(it, ignoreCase = true) }
    }

    private fun parseGovernment(input: String): String? {
        val governments = listOf(
            "민주공화정", "대통령공화정", "입헌군주정", "전제군주정",
            "권위주의 독재정", "군사독재정", "공산주의정", "신정정치", "기업국가"
        )
        val t = input.trim()
        val n = t.toIntOrNull()
        if (n != null && n in 1..governments.size) return governments[n - 1]
        return governments.firstOrNull { t.contains(it, ignoreCase = true) }
    }

    private fun parsePlanet(input: String, race: String): String? {
        val t = input.trim()
        if (t.length < 2) return null
        return t.take(60)
    }

    private enum class TraitCommandResult {
        UPDATED, CONFIRMED, INVALID
    }

    private fun usedTraitPoints(): Int = traitLevels.sum()

    private fun traitSummary(): String =
        traitNames.indices
            .filter { traitLevels[it] > 0 }
            .joinToString(", ") { idx ->
                "${traitNames[idx]} Lv.${traitLevels[idx]}"
            }
            .ifBlank { "미분배" }

    private fun handleTraitCommand(input: String): TraitCommandResult {
        val raw = input.trim()
        if (raw.isBlank()) return TraitCommandResult.INVALID

        if (raw.equals("초기화", ignoreCase = true) ||
            raw.equals("reset", ignoreCase = true)
        ) {
            traitLevels.fill(0)
            return TraitCommandResult.UPDATED
        }

        if (
            raw == "확정" ||
            raw == "완료" ||
            raw == "다음"
        ) {
            return if (usedTraitPoints() == 12) {
                TraitCommandResult.CONFIRMED
            } else {
                TraitCommandResult.INVALID
            }
        }

        val working = traitLevels.copyOf()

        val tokens = raw
            .replace("，", ",")
            .split(",", " ")
            .map { it.trim() }
            .filter { it.isNotBlank() }

        if (tokens.isEmpty()) return TraitCommandResult.INVALID

        var changed = false

        for (token in tokens) {
            Regex("""^([1-8])\+$""").find(token)?.let { m ->
                val idx = m.groupValues[1].toInt() - 1
                if (working[idx] >= 5) return TraitCommandResult.INVALID
                working[idx] += 1
                changed = true
                return@let
            } ?: run {
                Regex("""^([1-8])-$""").find(token)?.let { m ->
                    val idx = m.groupValues[1].toInt() - 1
                    if (working[idx] <= 0) return TraitCommandResult.INVALID
                    working[idx] -= 1
                    changed = true
                    return@let
                } ?: run {
                    val set = Regex("""^([1-8])[:=]([0-5])$""").find(token)
                        ?: return TraitCommandResult.INVALID
                    val idx = set.groupValues[1].toInt() - 1
                    val level = set.groupValues[2].toInt()
                    working[idx] = level
                    changed = true
                }
            }
        }

        if (!changed) return TraitCommandResult.INVALID
        if (working.sum() > 12) return TraitCommandResult.INVALID

        for (i in working.indices) traitLevels[i] = working[i]
        return TraitCommandResult.UPDATED
    }

    private fun saveSetupState() {
        prefs.edit()
            .putString(KEY_SETUP_STAGE, setupStage.name)
            .putString(KEY_RACE, selectedRace)
            .putString(KEY_NATION, nationName)
            .putString(KEY_GOVERNMENT, selectedGovernment)
            .putString(KEY_PLANET, selectedPlanet)
            .putString(KEY_TRAITS, selectedTraits)
            .putString(KEY_TRAIT_LEVELS, traitLevels.joinToString(","))
            .apply()
    }

    private fun restoreSetupState() {
        setupStage = runCatching {
            SetupStage.valueOf(prefs.getString(KEY_SETUP_STAGE, SetupStage.NONE.name) ?: SetupStage.NONE.name)
        }.getOrDefault(SetupStage.NONE)

        selectedRace = prefs.getString(KEY_RACE, "") ?: ""
        nationName = prefs.getString(KEY_NATION, "") ?: ""
        selectedGovernment = prefs.getString(KEY_GOVERNMENT, "") ?: ""
        selectedPlanet = prefs.getString(KEY_PLANET, "") ?: ""
        selectedTraits = prefs.getString(KEY_TRAITS, "") ?: ""
        restoreTraitLevels(prefs.getString(KEY_TRAIT_LEVELS, null))
    }

    private fun restoreTraitLevels(raw: String?) {
        traitLevels.fill(0)
        if (raw.isNullOrBlank()) return

        val values = raw.split(",")
        for (i in 0 until minOf(values.size, traitLevels.size)) {
            traitLevels[i] = values[i].toIntOrNull()?.coerceIn(0, 5) ?: 0
        }

        if (traitLevels.sum() > 12) {
            traitLevels.fill(0)
        }
    }

    private fun sendGameMessage(userMsg: String, displayUser: Boolean) {
        if (generationJob?.isActive == true) return
        if (displayUser) addMessage(Message(UUID.randomUUID().toString(), userMsg, true))
        rawAssistant.clear()
        lastUiRenderMs = 0L
        addMessage(Message(UUID.randomUUID().toString(), "", false), persist = false)
        setBusyUi("GM이 응답 생성 중…")

        val modelPrompt = buildModelPrompt(userMsg)
        generationJob = lifecycleScope.launch(Dispatchers.Default) {
            engine.sendUserPrompt(modelPrompt)
                .onCompletion {
                    withContext(Dispatchers.Main) {
                        needsCoreInjection = false
                        needsResumeContext = false
                        val visible = stripHiddenThinking(rawAssistant.toString()).trim()
                        replaceLastAssistant(if (visible.isBlank()) "응답을 생성하지 못했습니다. 같은 행동을 다시 입력해주세요." else visible)
                        saveAutosave()
                        setIdleUi()
                    }
                }
                .collect { token ->
                    rawAssistant.append(token)

                    val now = System.currentTimeMillis()
                    if (now - lastUiRenderMs >= 180L) {
                        lastUiRenderMs = now
                        val visible = stripHiddenThinking(rawAssistant.toString())
                        withContext(Dispatchers.Main) {
                            replaceLastAssistant(visible)
                        }
                    }
                }
        }
    }

    private fun buildModelPrompt(userMsg: String): String {
        val out = StringBuilder()

        out.append("/no_think\n")
        out.append(
            """
너는 'SF 우주 제국 시뮬레이션 v6.5'의 한국어 GM이다.

절대 규칙:
- 앱에서 이미 확정된 국가 생성값을 절대로 변경하지 않는다.
- '새 게임' 버튼이 눌리지 않은 한 종족 선택/국가 생성/세계 초기화를 다시 시작하지 않는다.
- 현재 setupStage가 PLAYING이면 캠페인은 이미 시작된 상태다.
- PLAYING 상태에서는 "게임 시작", "모태양계 생성", "AI 8국가 초기화", "프롤로그 재생성"을 반복하지 않는다.
- 플레이어가 현재 행동을 입력하면 기존 세계의 다음 결과만 처리한다.
- 미접촉 AI 국가의 비공개 정보는 노출하지 않는다.
- 성간항행 이전에는 다른 태양계와 직접 외교/무역/전쟁할 수 없다.
- 단순 질문은 시간을 진행하지 않는다.
- 실제 행동이 수행된 경우에만 시간을 진행한다.
- 내부 사고과정, <think>, <analysis>는 출력하지 않는다.
- 모바일 화면에 맞게 짧고 읽기 쉬운 한국어로 답한다.
""".trimIndent()
        )
        out.append("\n\n")

        if (setupStage == SetupStage.PLAYING && selectedRace.isNotBlank()) {
            out.append("[현재 캠페인 상태]\n")
            out.append("캠페인 진행 상태: 이미 시작됨(PLAYING)\n")
            out.append("국가 생성 및 세계 초기화: 완료됨\n")
            out.append("절대 다시 초기화하지 말 것\n\n")

            out.append("[절대 변경 금지 · 확정 국가 설정]\n")
                .append(confirmedSetupSummary())
                .append("\n\n")
        }

        // 매 요청마다 최근 실제 대화를 넣어 작은 로컬 모델의 상태 이탈을 막는다.
        if (messages.isNotEmpty()) {
            out.append("[최근 게임 기록]\n")
            messages.takeLast(8).forEach { m ->
                val content = m.content.trim()
                if (content.isNotBlank()) {
                    out.append(if (m.isUser) "플레이어: " else "GM: ")
                        .append(content.take(900))
                        .append('\n')
                }
            }
            out.append('\n')
        }

        val details = relevantRuleSnippet(userMsg).take(2200)
        if (details.isNotBlank()) {
            out.append("[이번 행동 관련 규칙]\n")
                .append(details)
                .append("\n\n")
        }

        out.append("[현재 플레이어 입력]\n")
            .append(userMsg)
            .append("\n\n")

        if (setupStage == SetupStage.PLAYING) {
            out.append(
                """
지시:
- 위 입력을 이미 진행 중인 캠페인의 다음 행동으로 처리한다.
- 과거 설정을 다시 만들지 않는다.
- 세계를 리셋하지 않는다.
- 이미 끝난 프롤로그를 반복하지 않는다.
- 현재 행동의 결과, 핵심 변화, 다음 선택지만 출력한다.
""".trimIndent()
            )
        } else {
            out.append("현재 앱이 지정한 국가 생성 단계만 처리한다.")
        }

        out.append("\n/no_think\n최종 게임 본문만 한국어로 출력한다.")
        return out.toString()
    }

    private fun relevantRuleSnippet(input: String): String {
        val q = input.lowercase()
        val titles = mutableListOf<String>()
        fun hit(vararg words: String) = words.any { q.contains(it) }
        if (hit("종족", "인류", "사이보그", "노바인", "드라칸", "실리카", "테리안", "아쿠아리", "바이오닉", "제로니안")) titles += listOf("3. 종족 체계", "4. 종족 추가 특성")
        if (hit("정부", "정치", "공화국", "독재", "기업", "집단의식", "제정", "신정", "연방")) titles += "5. 정치체계"
        if (hit("항행", "ftl", "성간", "탐사", "첫 접촉", "외계")) titles += listOf("5-C. 항행 기술과 성간 진출", "5-D. 첫 접촉과 미발견 문명")
        if (hit("인구", "식량", "성장")) titles += "6. 인구 시스템"
        if (hit("행성", "식민", "적합도", "지구", "광물", "철광석", "티타늄", "수소", "에너지")) titles += listOf("7. 행성 유형", "9. 종족-행성 적합도 판정", "10. 행성 지구와 경제 생산")
        if (hit("함선", "함대", "전투", "전쟁", "조선", "건조")) titles += listOf("12. 함선 규모등급과 함종", "13. 함선 전투력·건조·유지 기준", "14. 함선 건조 조건")
        if (hit("연구", "기술", "rp", "초공간", "고대기술")) titles += "15. 연구 시스템"
        if (hit("외교", "관계", "동맹", "협상", "무역")) titles += "16. 외교 시스템"
        if (hit("안정", "반란", "치안")) titles += "18. 안정도·치안"
        if (hit("네메시스", "기계의 부상", "2250")) titles += "19. 메인 이벤트"
        if (hit("상태창", "현재 상태", "현황")) titles += "23. 상태창"
        if (titles.isEmpty()) return ""
        return titles.distinct().joinToString("\n\n") { extractSection(it) }.take(1800)
    }

    private fun extractSection(titleNeedle: String): String {
        val lines = fullRules.lines()
        var start = -1
        for (i in lines.indices) {
            if (lines[i].contains(titleNeedle, ignoreCase = true)) { start = i; break }
        }
        if (start < 0) return ""
        val out = StringBuilder()
        for (i in start until lines.size) {
            if (i > start && lines[i].startsWith("====") && out.isNotEmpty()) break
            out.append(lines[i]).append('\n')
        }
        return out.toString().trim()
    }

    private fun stripHiddenThinking(raw: String): String {
        var s = raw
        s = s.replace(Regex("(?is)<think>.*?</think>"), "")
        s = s.replace(Regex("(?is)<analysis>.*?</analysis>"), "")
        val thinkOpen = s.indexOf("<think>", ignoreCase = true)
        if (thinkOpen >= 0) s = s.substring(0, thinkOpen)
        val analysisOpen = s.indexOf("<analysis>", ignoreCase = true)
        if (analysisOpen >= 0) s = s.substring(0, analysisOpen)
        return s.replace(Regex("(?is)</?(think|analysis)>"), "").trimStart()
    }

    private fun addAssistant(text: String, persist: Boolean = true) = addMessage(Message(UUID.randomUUID().toString(), text, false), persist)

    private fun addMessage(message: Message, persist: Boolean = true) {
        messages.add(message)
        messageAdapter.notifyItemInserted(messages.lastIndex)
        messagesRv.scrollToPosition(messages.lastIndex)
        if (persist) saveAutosave()
    }

    private fun replaceLastAssistant(content: String) {
        if (messages.isEmpty() || messages.last().isUser) return
        messages[messages.lastIndex] = messages.last().copy(content = content)
        messageAdapter.notifyItemChanged(messages.lastIndex)
        messagesRv.scrollToPosition(messages.lastIndex)
    }

    private fun saveSnapshot() {
        prefs.edit()
            .putString(KEY_SLOT, serializeMessages())
            .putString(KEY_SLOT_SETUP, serializeSetupState())
            .apply()
        toast("게임을 저장했습니다.")
    }

    private fun loadSnapshot() {
        val json = prefs.getString(KEY_SLOT, null)
        if (json.isNullOrBlank()) { toast("저장된 게임이 없습니다."); return }
        restoreMessages(json)
        prefs.getString(KEY_SLOT_SETUP, null)?.let { restoreSetupStateFromJson(it) }
        needsCoreInjection = true
        needsResumeContext = true
        val path = currentModelPath
        if (isModelReady && path != null) {
            lifecycleScope.launch(Dispatchers.IO) {
                try {
                    withContext(Dispatchers.Main) { setBusyUi("세이브 복구 중…") }
                    resetLoadedModel(path)
                    withContext(Dispatchers.Main) { setIdleUi(); toast("세이브를 불러왔습니다.") }
                } catch (t: Throwable) {
                    withContext(Dispatchers.Main) { setIdleUi(); toast("세이브 복구 중 모델 초기화 실패") }
                }
            }
        } else toast("세이브를 불러왔습니다. 모델을 선택하세요.")
    }

    private fun saveAutosave() {
        prefs.edit().putString(KEY_AUTOSAVE, serializeMessages()).apply()
        saveSetupState()
    }

    private fun restoreAutosave() {
        val json = prefs.getString(KEY_AUTOSAVE, null) ?: return
        restoreMessages(json)
        needsResumeContext = messages.isNotEmpty()
    }

    private fun serializeMessages(): String {
        val arr = JSONArray()
        messages.takeLast(MAX_SAVED_MESSAGES).forEach { m ->
            arr.put(JSONObject().put("id", m.id).put("content", m.content).put("isUser", m.isUser))
        }
        return arr.toString()
    }

    private fun restoreMessages(json: String) {
        try {
            val arr = JSONArray(json)
            messages.clear()
            for (i in 0 until arr.length()) {
                val o = arr.getJSONObject(i)
                messages.add(Message(o.optString("id", UUID.randomUUID().toString()), o.optString("content"), o.optBoolean("isUser")))
            }
            messageAdapter.notifyDataSetChanged()
            if (messages.isNotEmpty()) messagesRv.scrollToPosition(messages.lastIndex)
            saveAutosave()
        } catch (t: Throwable) {
            Log.e(TAG, "Restore failed", t)
            toast("세이브 파일을 읽지 못했습니다.")
        }
    }

    private fun serializeSetupState(): String = JSONObject()
        .put("stage", setupStage.name)
        .put("race", selectedRace)
        .put("nation", nationName)
        .put("government", selectedGovernment)
        .put("planet", selectedPlanet)
        .put("traits", selectedTraits)
        .put("traitLevels", traitLevels.joinToString(","))
        .toString()

    private fun restoreSetupStateFromJson(json: String) {
        runCatching {
            val o = JSONObject(json)
            setupStage = SetupStage.valueOf(o.optString("stage", SetupStage.NONE.name))
            selectedRace = o.optString("race", "")
            nationName = o.optString("nation", "")
            selectedGovernment = o.optString("government", "")
            selectedPlanet = o.optString("planet", "")
            selectedTraits = o.optString("traits", "")
            restoreTraitLevels(o.optString("traitLevels", ""))
            saveSetupState()
        }
    }

    private fun readAssetText(name: String): String = assets.open(name).bufferedReader().use { it.readText() }

    private fun setBusyUi(status: String) {
        setStatus(status)
        userInputEt.isEnabled = false
        userActionFab.isEnabled = false
        selectModelBtn.isEnabled = false
        saveBtn.isEnabled = false
        loadBtn.isEnabled = false
        newGameBtn.isEnabled = false
    }

    private fun setIdleUi() {
        userInputEt.isEnabled = isModelReady
        userActionFab.isEnabled = isModelReady
        refreshButtons()
    }

    private fun refreshButtons() {
        selectModelBtn.isEnabled = generationJob?.isActive != true
        saveBtn.isEnabled = messages.isNotEmpty() && generationJob?.isActive != true
        loadBtn.isEnabled = !prefs.getString(KEY_SLOT, null).isNullOrBlank() && generationJob?.isActive != true
        newGameBtn.isEnabled = isModelReady && generationJob?.isActive != true
    }

    private fun setStatus(text: String) { statusTv.text = text }
    private fun toast(text: String) = Toast.makeText(this, text, Toast.LENGTH_SHORT).show()

    private fun ensureModelsDirectory() = File(filesDir, DIRECTORY_MODELS).also {
        if (it.exists() && !it.isDirectory) it.delete()
        if (!it.exists()) it.mkdir()
    }

    override fun onStop() {
        saveAutosave()
        super.onStop()
    }

    override fun onDestroy() {
        generationJob?.cancel()
        // InferenceEngine is a process-wide singleton.
        // Do not destroy it here because Android may recreate the Activity
        // while the process is still alive, leaving the singleton in ModelReady.
        super.onDestroy()
    }

    companion object {
        private val TAG = MainActivity::class.java.simpleName
        private const val DIRECTORY_MODELS = "models"
        private const val FILE_EXTENSION_GGUF = ".gguf"
        private const val PREFS = "sf_empire_offline"
        private const val KEY_MODEL_PATH = "model_path"
        private const val KEY_AUTOSAVE = "autosave"
        private const val KEY_SLOT = "save_slot_1"
        private const val KEY_SLOT_SETUP = "save_slot_1_setup"
        private const val KEY_SETUP_STAGE = "setup_stage"
        private const val KEY_RACE = "setup_race"
        private const val KEY_NATION = "setup_nation"
        private const val KEY_GOVERNMENT = "setup_government"
        private const val KEY_PLANET = "setup_planet"
        private const val KEY_TRAITS = "setup_traits"
        private const val KEY_TRAIT_LEVELS = "setup_trait_levels"
        private const val MAX_SAVED_MESSAGES = 80
    }
}

fun GgufMetadata.filename() = when {
    basic.name != null -> basic.name?.let { name -> basic.sizeLabel?.let { size -> "$name-$size" } ?: name }
    architecture?.architecture != null -> architecture?.architecture?.let { arch -> basic.uuid?.let { uuid -> "$arch-$uuid" } ?: "$arch-${System.currentTimeMillis()}" }
    else -> "model-${System.currentTimeMillis()}"
}
