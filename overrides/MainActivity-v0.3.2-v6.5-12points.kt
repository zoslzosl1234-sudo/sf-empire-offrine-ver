...
class MainActivity : AppCompatActivity() {
    ...
    private fun handleSetupInput(input: String) {
        when (setupStage) {
            ...
            SetupStage.TRAITS -> {
                val traits = parseTraits(input)
                if (traits == null) {
                    addAssistant(
                        "특성 분배 형식이 잘못되었거나 총합이 12포인트가 아닙니다. 각 항목은 Lv.0~5, 전체 합계는 정확히 12로 입력해주세요.\n\n" +
                        traitsPrompt()
                    )
                    return
                }
...
5. **산업** — IP·건설·생산
6. **사회/행정** — 안정·행정·LC 효율
7. **감각/정보** — 탐지·정찰·이상현상 발견
8. **에너지/회복** — 에너지 생산·운용 또는 회복·재난저항

입력 예:
`1:2, 4:3, 6:1`

총 **12포인트**를 분배하세요. 각 특성은 **Lv.0~Lv.5**이며, 입력한 레벨의 합이 정확히 12여야 합니다.
""".trimIndent()
...
""".trimIndent()
    ...
    private fun parseTraits(input: String): String? {
        val names = listOf(
            "적응", "번성", "강인", "지능",
        ...
        for (part in parts) {
            val match = Regex("""^\s*([1-8])\s*[:=]\s*([0-5])\s*$""").find(part)
                ?: return null
        ...
        if (selected.values.sum() != 12) return null

        return selected.entries.joinToString(", ") { (idx, level) ->
            "${names[idx]} Lv.$level"
        }
    }
